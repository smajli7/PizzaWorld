...
@Repository
public interface PizzaRepo extends JpaRepository<Order, Long> {
    ...
    @Query("SELECT o FROM Order o WHERE (:customerId IS NULL OR o.customerId = :customerId) AND (:storeId IS NULL OR o.storeId = :storeId)")
    List<Order> dynamicOrderFilter(@Param("customerId") String customerId, @Param("storeId") String storeId);
}
