...
-- These views complement your existing view structure and provide
-- role-based data access for better performance
...
JOIN orders o ON oi.orderid = o.orderid
JOIN stores s ON o.storeid = s.storeid
GROUP BY s.state_abbr, o.storeid, p.sku, p.name, p.category, p.size, p.price
ORDER BY state, store_id, total_revenue DESC;
...
WHERE o.customerid NOT IN (
    ...
    WHERE s2.state_abbr = s.state_abbr
    AND o2.orderdate < DATE_TRUNC('month', o.orderdate)
)
GROUP BY s.state_abbr, EXTRACT(YEAR FROM o.orderdate), EXTRACT(MONTH FROM o.orderdate), TO_CHAR(o.orderdate, 'Month')
ORDER BY state, year DESC, month DESC;
...
JOIN products p ON oi.sku = p.sku
JOIN stores s ON o.storeid = s.storeid
GROUP BY s.state_abbr, o.storeid, p.category
ORDER BY state, store_id, total_revenue DESC;
